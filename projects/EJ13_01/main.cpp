...
Camera cameraVV(glm::vec3(0.0f, 0.0f, 3.0f));
...
void render(const Geometry& quad, const Geometry& cube, const Shader& s_phong, const Shader& s_fbo,
	...
	glm::mat4 viewVV = cameraVV.getViewMatrix();
	glm::mat4 projVV = glm::perspective(glm::radians(45.0f), static_cast<float>(Window::instance()->getWidth()) / Window::instance()->getHeight(), 0.1f, 100.0f);
	...
	glm::mat3 normalMat1 = glm::inverse(glm::transpose(glm::mat3(model1)));
	...
	s_phong.set("view", viewVV);
	s_phong.set("proj", projVV);

	s_phong.set("viewPos", cameraVV.getPosition());

	s_phong.set("model", model1);
	s_phong.set("normalMat", normalMat1);
	quad.render();


	s_phong.set("model", model2);
	s_phong.set("normalMat", normalMat2);
