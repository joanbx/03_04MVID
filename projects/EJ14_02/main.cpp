...
SpotLight* spotLights = new SpotLight[nSpotLight]{
	...
	{glm::vec3(3.0f,5.0f,0.0f),glm::vec3(-0.75f, -1.0f, 0.0f), glm::vec3(0.1f,0.1f,0.1f), glm::vec3(0.5f,0.5f,0.5f),glm::vec3(1.0f,1.0f,1.0f), 1.0f, 0.01f, 0.016f, 35.0, 45.0, glm::vec3(1.0f,1.0f,1.0f)}
};
...
void render(const Geometry& quad, const Geometry& cube, const Geometry& sphere,
	...
	const Texture& t_albedo, const Texture& t_specular, std::vector<uint32_t> fbo, std::vector<uint32_t> fbo_texture) {
	...
	for (uint16_t i = 0; i < nSpotLight; ++i) {
		//FIRST PASS
		...
		const glm::mat4 lightView = glm::lookAt(spotLights[i].getPosition(), spotLights[i].getPosition() + spotLights[i].getDirection(), glm::vec3(0, 1, 0));
...
//SECOND PASS
	...
	s_phong.set("lightSpaceMatrix0", lightSpaceMatrixs[0]);
	s_phong.set("lightSpaceMatrix1", lightSpaceMatrixs[1]);
...
int main(int, char* []) {
	...
	std::vector<uint32_t> fbo;
	fbo.push_back(fbo1.first);
	fbo.push_back(fbo2.first);
	std::vector<uint32_t> fboTexture;
	fboTexture.push_back(fbo1.second);
	fboTexture.push_back(fbo2.second);
	...
	while (window->alive()) {
		...
		render(quad, cube, sphere, s_phong, s_depth, s_debug, s_light, t_albedo, t_specular, fbo, fboTexture);
		window->frame();
	}
