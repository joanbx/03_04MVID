...
void render(const Geometry& quad, const Geometry& cube, const Shader& s_phong, const Shader& s_fbo,
	const Texture& t_albedo, const Texture& t_specular, const uint32_t fbo, const uint32_t fbo_texture, const uint32_t fbo2, const uint32_t fbo2_texture) {
	...
	glBindFramebuffer(GL_FRAMEBUFFER, fbo2); //Bindear segundo fbo y bibujar la primera pasada de Horizontal
	...
	s_fbo.set("offset", 1.0f / (Window::instance()->getWidth()), 0.0f); //Offset Horizontal
	s_fbo.set("screenTexture", 0);
	quad.render();

	glBindFramebuffer(GL_FRAMEBUFFER, 0); //Dibujar contenido segundo fbo y hacer la segunda pasada de Horizontal
	glClear(GL_COLOR_BUFFER_BIT);

	glBindTexture(GL_TEXTURE_2D, fbo2_texture);
	s_fbo.set("offset", 0.0f , 1.0f / (Window::instance()->getHeight())); //Offset Vertical
	quad.render();
...
int main(int, char* []) {
	...
	const Shader s_fbo("../projects/EJ13_03/fbo.vs", "../projects/EJ13_03/fbo.fs"); //<-Ver Shader para comprobar el efecto Sobel
	...
	while (window->alive()) {
		...
		render(quad, cube, s_phong, s_fbo, t_albedo, t_specular, std::get<0>(fbo1), std::get<1>(fbo1), std::get<0>(fbo2), std::get<1>(fbo2));
		window->frame();
	}
