...
glm::vec3 lightDirSpot(0.0f, -1.0f, 0.0f);
...
void render(const Geometry& floor, const Geometry& object, const Geometry& light, const Shader& s_phong, const Shader& s_light,
	...
	for (uint32_t i = 0; i < pointLightPositions.size(); ++i) {
		...
		s_phong.set((lightName + "diffuse").c_str(), 0.2f, 0.2f, 0.2f); //Blanquecino
	...
	for (uint32_t i = 0; i < spotLightPositions.size(); ++i) {
		...
		s_phong.set((lightName + "linear").c_str(), 0.09f);
		s_phong.set((lightName + "quadratic").c_str(), 0.032f);
		s_phong.set((lightName + "cutOff").c_str(), glm::cos(glm::radians(30.0f)));
		s_phong.set((lightName + "outerCutOff").c_str(), glm::cos(glm::radians(40.0f)));
	}
