...
float mixTex = 0.5f; //Valor inicial de la interpolación de textura
...
void handleInput() {
	...
	for (auto& key : keys) {
		...
		if (key.second == 1 && (key.first == 263 || key.first == 333)) { //Left Arrow / -
			mixTex -= 0.05;
			if (mixTex <= 0) mixTex = 0;
			//std::cout << mixTex << std::endl;
		}

		if (key.second == 1 && (key.first == 262 || key.first == 334)) { //Right Arrow / +
			mixTex += 0.05;
			if (mixTex >= 1) mixTex = 1;
			//std::cout << mixTex << std::endl;
		}
...
void render(uint32_t VAO, const Shader& shader, uint32_t tex1, uint32_t tex2) {
	...
	shader.set("tex_1", 0);
	shader.set("tex_2", 1);
	shader.set("mixTex",mixTex); //Controlado con teclas Derecha e Izquierda o + y -
